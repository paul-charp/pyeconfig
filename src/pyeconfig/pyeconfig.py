...
def get_config(config_file: str, keys: list[str] = list()) -> dict[str]:
    ...
    Args:
        ...
        keys (list[str], optional): Name of variable to look for in the config file and in the environment. If not provided the keys will be retrieved from the constants present in the config file.
    ...
    Returns:
        dict[str]: A dictionary containing the configuration merged from the config file and overridden by the environment.
    """
    ...
    for key in keys:
        ...
        if not os.environ.get(key) == None:
            ...
            if os.pathsep in value or type(getattr(config_mod, key)) is list:
